...
def renderTest():
    ...
    # Setting Material
    ground = rtm.Dielectric(rtu.Color(0.270, 0.667, 0.9),0.33)
    red_mat = rtl.Diffuse_light(rtu.Color(1,0,0))
    blue_mat = rtl.Diffuse_light(rtu.Color(0,0,1))
    ...
    # sphere left 
    left_centerP =rtu.Vec3(-1,0.5,-3)
    sphere_left = rto.Sphere(left_centerP,1,red_mat)
    sphere_left.add_moving(left_centerP + rtu.Vec3(1,0,0))
    world.add_object(sphere_left)
    
    # sphere left 
    right_centerP =rtu.Vec3(1,0.5,-3)
    sphere_right = rto.Sphere(right_centerP,1,blue_mat)
    sphere_right.add_moving(right_centerP + rtu.Vec3(-1,0,0))
